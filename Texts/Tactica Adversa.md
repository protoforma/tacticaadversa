...
ТАКТИКА АДВЕРЗА (TACTICA ADVERSA, ТА) - универсальный метод анализа созданный на основе   ОРИГИНАЛА, АКСИОМ и ГЕОМЕТРИЧЕСКИХ ПРИМИТИВОВ (набора обобщений доведенных до уровня абстракций). ТА включает в себя алгоритм принятия решений построенный на МАКСИМАХ. 

ТА является самым точным и полным (по детализации и степени охвата  исследуемого объекта) методом анализа и позволяет успешно работать с  любым объектом на физическом плане.
...
Несколько примеров (здесь и далее примеры приводятся на разных данных и типах графиков (первые два - свечи, третий - кривая, четвертый - крестики-нолики), чтобы продемонстрировать универсальность и робастность ТА) расположения CP (в программе Skilful Pro данная точка так и обозначается - CP). 
...
- **модель** (λ)
  ...
  - парабола - описывает резкую смену состояний объекта, катастрофу (в терминах Теории катастроф) (аналитическая)
  - 3МЛ - может состоять из 3 или 4 ММЛ (аналитическая)
- **спираль** (тонкий план)
- **протоформа** (тонкий план), на физическом - сценарий
