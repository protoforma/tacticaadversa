...
​	≈ - приблизительное равенство, точного соответствия не требуется. К МДР принято относить и все МР И МП для которых СТ МР или НР МП располагаются на расстоянии более семи отрезков от 1 точки модели до 4:

- |min^1^ - СТ| > 7 * |max^4^ - min^1^| или |НР - max^4^| > 7 * |max^4^ - min^1^| 

	для up-тренда

- |СТ - max^1^| > 7 * |max^1^ - min^4^| или |min^4^ - HP| > 7 * |max^1^ - min^4^|
