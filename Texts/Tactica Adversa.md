...
- λ~МР~ = min^1^ + max^2^ + min^3^ + max^4^ 

  ​	модель описывающая восходящий тренд (up-тренд), где

  - (min^1^ <= min^3^) < max^2^ < max^4^ и (min^1^ + max^2^) < (min^3^ + max^4^) 

  или

- λ~МР~ = max^1^ + min^2^ + max^3^ + min^4^ 
...
- (СТ + min^1^) > 7 * (min^1^ + max^4^) и (max^4^ + HP) > 7 * (min^1^ + max^4^) 
